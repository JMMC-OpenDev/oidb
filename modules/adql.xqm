...
declare %private function adql:from_clause($params) as xs:string {
    let $sql-table := adql:get-parameter($params, 'catalog', $config:sql-table)
    let $from := 'FROM ' || $sql-table || ' AS ' || $adql:correlation-name
    let $join := if( true() or adql:get-parameter($params, 'join', ()) ) then () else " LEFT JOIN "||$config:sql-datalink-table||" on " || $config:sql-datalink-table || ".id =" || $adql:correlation-name ||".id"
    return $from || $join
};
...
(:~
 ...
 : @return an ADQL SELECT statement
 :)
declare function adql:build-quickplots-query($params as item()*) as xs:string {
    let $query := adql:get-parameter($params, 'query', ())
    let $query := if ($query) then
        $query
    else
        ...
        string-join((
            ...
            adql:set_quantifier($params),
            $limits[1],
            $adql:correlation-name || '.' || "id", (: adql:select_list($params), :)
            adql:from_clause($params),
            adql:where_clause($params),
            adql:group_by($params),
            (: having_clause :)
    ...
    let $query := "SELECT * from " || $config:sql-datalink-table || " WHERE "||$config:sql-datalink-table || ".id in (" || $query || ") AND access_url LIKE '%.png'"

    let $log := util:log("info", "q: "|| $query)

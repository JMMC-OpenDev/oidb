...
let $all := map:merge(
    ...
    return
        ...
        let $query :=
            adql:build-query(('distinct', 'col=access_url', 'col=obs_release_date', 'col=datapi', 'col=obs_creator_name', 'col=progid', 'col=proposal_subid',
...
(:    let $row-lines := for $row at $pos in $rows :)
        ...
        let $progid := $data[5]
        let $proposal_subid := $data[6]


        let $cois := if ($public) then () else user:get-delegations($datapi)//delegation[collection=$collection or progid=$progid or proposal_subid=$proposal_subid]/alias
        let $emails := if ($public) then ()  else user:get-email(($datapi,$obs_creator-name, $cois))
...
(: Prepare datalink access list :)
    let $datalink-lines := for $row at $pos in $datalink-rows
        ...
        let $progid := $data[5]
        let $proposal_subid := $data[6]
        let $datalink_access_url := $data[7]
